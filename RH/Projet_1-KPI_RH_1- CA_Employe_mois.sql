...
*/
-- création de vue 
create view view_pbi_caemploye_mois as 
...
Select 
   ...
   ( Select 
    ...
    -- Calcul du montant de la variation CA N/ CA n-1
   ...
   -- Calcul du taux de variation CA N/ CA n-1
   (sum(reqA.CA_employe)/( Select 
							sum(reqP.CA_employe)
							from view_rh as reqP
								where 
...
group by lastName,firstName,officeCode,year(shippedDate),month(shippedDate)
