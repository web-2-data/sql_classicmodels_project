/* -- Script de création de la vue Chiffre d'affaires par mois  par année pour Power BI
 chiffre d'affaires par catégorie et par mois avec comparaison 
...
-- Création de la vue
CREATE VIEW view_pbi_caprod_mois as 
...
group by reqA.productLine, month(reqA.shippedDate),year(reqA.shippedDate)
-- trie par productLine, month, et year
order by reqA.productLine asc, month(reqA.shippedDate) asc, year(reqA.shippedDate);